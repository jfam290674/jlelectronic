...
class LeadViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self) -> QuerySet[Lead]:
        qs = Lead.objects.select_related("asesor", "cliente", "producto")
        ...
        if not is_admin(user):
            scope_q = Q(asesor=user)
            if _model_has_field(Lead, "owner") or hasattr(Lead, "owner_id"):
    ...
    def run_reminders(self, request):
        ...
        try:
            ...
            qs = self.get_queryset().filter(
                reminder_next_at__isnull=False,
                reminder_next_at__lte=today,
            ).exclude(etapa__in=["GANADO", "CANCELADO"])

            enviados = 0
            for lead in qs:
                destino = (getattr(lead.asesor, "email", "") or "").strip()
                if not destino:
                    continue
                ...
                body = (
                    ...
                    f"Por favor da seguimiento hoy. Este recordatorio se repetirá a diario hasta cerrar (GANADO/CANCELADO).\n"
                )
                try:
                    send_mail(subject, body, getattr(settings, "DEFAULT_FROM_EMAIL", None), [destino], fail_silently=True)
                    enviados += 1
                except Exception:
                    logger.exception("Error enviando recordatorio para lead %s", getattr(lead, "pk", "N/A"))

                lead.reminder_next_at = today + timedelta(days=1)
                try:
...
class LeadPhotoViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self) -> QuerySet[LeadPhoto]:
        ...
        user = self.request.user
        if not is_admin(user):
            qs = qs.filter(lead__asesor=user)
    ...
    def perform_create(self, serializer: LeadPhotoSerializer) -> None:
        ...
        try:
            ...
            if lead_obj and (not is_admin(user)) and lead_obj.asesor_id != user.id:
...
class LeadEditRequestViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if is_admin(user):
            return qs
        return qs.filter(Q(requester=user) | Q(lead__asesor=user))

    def perform_create(self, serializer: LeadEditRequestSerializer) -> None:
        req: LeadEditRequest = serializer.save()
        user = self.request.user
        if not is_admin(user) and req.lead.asesor_id != user.id:
            try:
                req.delete()
            except Exception:
