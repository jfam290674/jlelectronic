...
class Lead(models.Model):
    ...
    cliente = models.ForeignKey(
        ...
        on_delete=models.PROTECT,
        related_name="leads",
        help_text="Cliente tomado del módulo de clientes.",
    )
    ...
    # -------- Recordatorios --------
    reminder_next_at = models.DateField(
        null=True, blank=True, help_text="Fecha del próximo recordatorio (envío de cotización/seguimiento)."
    )
    ...
    def autofill_from_relations(self) -> None:
        ...
        - Contacto/Mail/Teléfono/Ciudad desde Cliente.
        """
        if self.producto:
            if not self.marca:
