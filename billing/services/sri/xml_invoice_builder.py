...
def _build_info_tributaria(invoice: Invoice) -> etree._Element:
    ...
    # Campos opcionales (contribuyenteEspecial, obligadoContabilidad, etc.) se pueden ajustar luego
    return info
...
def _build_info_factura(invoice: Invoice) -> etree._Element:
    ...
    # Campos opcionales según configuración de la empresa:
    # contribuyenteEspecial (si la empresa tiene un número asignado)
    contribuyente_especial = getattr(
        empresa, "contribuyente_especial_numero", None
    )
    ...
    # Pagos (por ahora: un solo pago por el total, contado)
