...
/* ================== Hooks de búsqueda ================== */
function useSearchClientes(query: string) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const r = await fetch(`/api/clientes/?search=${encodeURIComponent(q)}&page_size=10`, {
          credentials: "include",
          signal: ctl.signal,
        });
        if (r.ok) {
...
export default function FunnelLeadForm() {
  ...
  const clientes = useSearchClientes(clienteQuery);
  ...
  useEffect(() => {
    if (clienteSel) {
      setMail(clienteSel.email || "");
      setTelefono(clienteSel.celular || "");
      setCiudad(clienteSel.ciudad || "");
      if (!nombreOpp) setNombreOpp(clienteSel.nombre);
    }
  }, [clienteSel, nombreOpp]);
  ...
  const canContinueStep1 = useMemo(() => {
    ...
    if (nombreOpp.trim().length <= 2) return false;
    if (!mesCompra) return false;

    const hasValidLine = lines.some((ln) => {
      const qty = Number(ln.cantidadStr);
      const price = Number(ln.precioStr);
      return Number.isFinite(qty) && qty >= 1 && Number.isFinite(price) && price >= 0;
    });
    return hasValidLine;
  }, [clienteSel, nombreOpp, mesCompra, lines]);
  ...
  async function createLeadAndUploadPhotos() {
    ...
    setErr(null);
    try {
      if (!clienteSel) throw new Error("Selecciona un cliente antes de enviar.");
      ...
      const payload: CreateLeadDTO = {
        ...
        nombre_oportunidad: nombreOpp.trim(),
        contacto,
        cargo,
        mail,
        telefono,
        ...
        <>
                  ...
                  {!clienteSel ? (
                    <>
                      <input
                        ...
                        style={{ ...INPUT_STYLE }}
                        placeholder="Buscar por nombre, cédula/RUC, ciudad, email…"
                        aria-label="Buscar cliente"
                        aria-required
                      />
                      <p className="text-[12px] mt-1" style={{ color: THEME.muted }}>
                        Selecciona un cliente existente. Si no aparece, revisa el registro de clientes.
                      </p>
                      {clienteQuery && (
                        <div className={DROPDOWN_CLASS} style={{ ...CARD_STYLE }}>
                          {clientes.length === 0 && (
                            <div className="px-3 py-2 text-sm" style={{ color: THEME.muted }}>
                              Sin resultados…
                            </div>
                          )}
                          {clientes.map((c) => (
                            <button
                              ...
                              onClick={() => {
                                setClienteSel(c);
                                setClienteQuery(`${c.nombre} (${c.identificador})`);
                              }}
                            ...
                            >
                              ...
                              <div className="text-xs" style={{ color: THEME.muted }}>
                                {c.identificador} · {c.ciudad || "s/ciudad"} · {c.email || "s/email"}
                              </div>
                            </button>
                          ))}
                        </div>
                    ...
                    <div
                      className="mt-1 rounded-xl border px-3 py-2 text-sm flex items-center justify-between"
                      style={{ ...CARD_STYLE }}
                    >
                      <div>
                        <div className="font-medium">{clienteSel.nombre}</div>
                        <div className="text-xs" style={{ color: THEME.muted }}>
                          {clienteSel.identificador} · {clienteSel.ciudad || "s/ciudad"} · {clienteSel.email || "s/email"}
                        </div>
                      </div>
                      <button
                        type="button"
          ...
          {loading && (
            <div className="fixed inset-0 bg-black/20 flex items-center justify-center">
              <div className="rounded-xl p-3 text-sm border shadow-sm" style={{ background: THEME.surface, color: THEME.text, borderColor: THEME.border }}>
                Procesando…
              </div>
            </div>
          )}
