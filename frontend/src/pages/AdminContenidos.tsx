...
export default function AdminContenidos() {
  ...
  /* ===== Estado global UI ===== */
  const [tab, setTab] = useState<"videos" | "manuales" | "marcas" | "modelos">("videos");
  ...
  }, [tab, marcaSelModelos, marcaSelVideo, marcaSelManual]);

  /* ===== Listas de videos / manuales ===== */
  useEffect(() => {
  ...
  }, [tab, marcaSelVideo, modeloSelVideo, marcaSelManual, modeloSelManual]);
      ...
      <div className="sticky top-0 z-10 text-white bg-gradient-to-b from-[#0A3D91] to-[#1B6DD8]">
        <div className="mx-auto max-w-6xl px-4 py-3 md:py-4">
          ...
          <div className="mt-3 flex gap-2 overflow-x-auto no-scrollbar" role="tablist" aria-label="Secciones">
            {(["videos", "manuales", "marcas", "modelos"] as const).map((k) => (
              <button
                role="tab"
                aria-selected={tab === k}
                key={k}
              ...
              >
                {k === "videos" ? "Videos" : k === "manuales" ? "Manuales" : k === "marcas" ? "Marcas" : "Modelos"}
              </button>
            ))}
          </div>
        </div>
