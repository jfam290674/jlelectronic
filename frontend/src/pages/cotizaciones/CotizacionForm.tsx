...
export default function CotizacionForm() {
  ...
  const [showImgEditor, setShowImgEditor] = useState<Record<number, boolean>>({});
  ...
  const COMPANY = {
    ...
    L4: "Cuenca - Ecuador",
    LOGO_WEB_PRIMARY: "/static/images/logo.png",
    LOGO_WEB_FALLBACK: "/static/images/logo.png",
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 sm:p-6 overflow-x-hidden pb-20 md:pb-6">
            ...
            return (
              <div key={i} className="rounded-2xl border p-3">
                <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
                  ...
                  <div className="md:col-span-6">
                        ...
                        {loadingRow ? (
                          ...
                          results.map((p) => (
                            <button
                              ...
                              className="w-full text-left px-3 py-2 text-sm hover:bg-slate-50"
                            >
                              <div className="font-medium">{pickName(p) || `ID ${p.id}`}</div>
                              <div className="text-xs text-slate-500">
                                {pickCategory(p)} {pickFeatures(p) ? `• ${pickFeatures(p)?.slice(0, 70)}…` : ""}
                              </div>
                            </button>
                          ))
                        )}
                    ...
                    <div className="mt-2 grid grid-cols-1 gap-2">
                      <input
                        ...
                        placeholder="Nombre del ítem (Nombre Modelo)"
                        value={it.producto_nombre}
                        onChange={(e) => setItem(i, { producto_nombre: e.target.value })}
                        aria-label={`Nombre del ítem fila ${i + 1}`}
                      />
                      <textarea
                        ...
                        aria-label={`Descripción del ítem fila ${i + 1}`}
                      />
                      <details
                        className="text-sm"
                        open={!!showImgEditor[i]}
                        onToggle={(e) =>
                          setShowImgEditor((old) => ({ ...old, [i]: (e.target as HTMLDetailsElement).open }))
                        }
                      >
                        <summary className="cursor-pointer select-none text-slate-600">
                          {showImgEditor[i] ? "Ocultar" : "Editar"} imagen (opcional)
                        </summary>
                        <input
                          className="mt-1 w-full rounded-xl border p-2 text-sm"
                          placeholder="URL de imagen"
                          value={it.producto_imagen_url || ""}
                          onChange={(e) => setItem(i, { producto_imagen_url: e.target.value })}
                          aria-label={`URL de imagen fila ${i + 1}`}
                        />
                      </details>
                    </div>
                  </div>
                  ...
                  <div className="md:col-span-4 grid grid-cols-3 gap-2 content-start">
                    <div>
                    ...
                    <div>
                      <label className="text-sm text-slate-600">P. unitario</label>
                      <input
                        type="number"
                        step="0.01"
                        className="w-full rounded-xl border p-2 focus:outline-none"
                        ...
                        aria-label={`Precio unitario fila ${i + 1}`}
                      />
                    </div>
                    <div>
                      <label className="text-sm text-slate-600">Total</label>
                      <div className="w-full rounded-xl border p-2 bg-slate-50">
                    ...
                    <div className="col-span-3 flex justify-end">
                      <button
                        ...
                        className="mt-2 px-3 py-2 rounded-xl border hover:bg-slate-50 inline-flex items-center gap-2 text-rose-600"
                        title="Quitar ítem"
                        aria-label={`Quitar ítem fila ${i + 1}`}
                      >
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="rounded-2xl border p-4">
          <div className="text-sm font-medium text-ink">Condiciones de la oferta</div>
              ...
              <div className="mx-auto max-w-4xl bg-white text-black">
                  ...
                  <div className="p-4 sm:p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="rounded-xl border p-3">
                      ...
                      <div className="mt-1 text-sm">
                        ...
                        {clienteSel?.identificador ? <div className="mt-0.5">Identificador: {clienteSel.identificador}</div> : null}
                ...
                <div className="mt-6 overflow-x-auto">
                  <table className="w-full text-sm border-spacing-0">
                    <thead>
                      <tr>
                        <th className="text-left px-2 py-2 text-white" style={{ background: brandBlue }}>Ítem</th>
                        <th className="text-left px-2 py-2 text-white" style={{ background: brandBlue }}>Descripción</th>
                        <th className="text-right px-2 py-2 text-white" style={{ background: brandBlue }}>Cant.</th>
                        <th className="text-right px-2 py-2 text-white" style={{ background: brandBlue }}>P. Unit.</th>
                        <th className="text-right px-2 py-2 text-white" style={{ background: brandBlue }}>Total</th>
                      </tr>
                    </thead>
                    <tbody>
                      {form.items.map((it, i) => (
                        <tr key={i} className="border-b" style={{ borderColor: `${brandBlue}22` }}>
                          ...
                          <td className="px-2 py-3 align-top">
                            <div className="font-medium" style={{ color: brandBlue }}>{it.producto_nombre || "—"}</div>
                            <div className="text-xs text-slate-600">
                              {it.producto_categoria ? (
                                <span className="inline-block px-1.5 py-0.5 rounded-md mr-1" style={{ background: `${brandOrange}14`, color: brandOrange }}>
                                  {it.producto_categoria}
                                </span>
                              ) : null}
                          ...
                          <td className="px-2 py-3 text-right align-top">{Number(it.cantidad || 0)}</td>
                          <td className="px-2 py-3 text-right align-top">${money(it.precio_unitario)}</td>
                ...
                <div className="mt-6 flex justify-end">
                  <div className="w-full sm:w-96 rounded-2xl overflow-hidden shadow-sm border" style={{ borderColor: `${brandBlue}22` }}>
                    <div className="px-4 py-2 text-white font-semibold" style={{ background: brandBlue }}>Resumen</div>
                    <div className="px-4 py-3 space-y-2 text-sm bg-white">
                      <div className="flex justify-between">
                        <div className="text-slate-600">Subtotal</div>
                        <div className="font-medium">${money(form.subtotal)}</div>
                      </div>
                      <div className="flex justify-between">
                        <div className="text-slate-600">Descuento ({ensureNumber(form.descuento_cliente_percent, 0)}%)</div>
                        <div className="font-medium" style={{ color: brandOrange }}>
                          -${money(form.descuento_total)}
                        </div>
                      </div>
                      <div className="flex justify-between">
                        <div className="text-slate-600">IVA ({ensureNumber(form.iva_percent, 15)}%)</div>
                        <div className="font-medium">${money(form.iva_total)}</div>
                      </div>
                      <div className="pt-2 mt-1 border-t flex items-center justify-between text-base">
                        <div className="font-semibold" style={{ color: brandBlue }}>TOTAL</div>
                ...
                {/* Datos bancarios (TEXTO, sin imagen) */}
                <div className="mt-6">
      ...
      <div className="md:hidden fixed bottom-0 inset-x-0 z-40 border-t bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/70">
        <div className="max-w-6xl mx-auto p-3 flex items-center gap-2">
          <Link
            to="/cotizaciones"
            className="flex-1 px-3 py-2 rounded-xl border bg-white hover:bg-slate-50 text-slate-700 inline-flex items-center justify-center gap-2 text-sm"
          ...
          <button
            onClick={() => setPreviewOpen(true)}
            className="px-3 py-2 rounded-xl border bg-white hover:bg-slate-50 text-slate-700 text-sm"
          ...
          <button
            onClick={handleSave}
            disabled={saving || !canSave}
            className="flex-1 px-3 py-2 rounded-xl text-white text-sm inline-flex items-center justify-center gap-2 disabled:opacity-60"
            style={{ background: brandBlue }}
