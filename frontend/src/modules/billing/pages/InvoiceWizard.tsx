...
// Producto: flexible para adaptarse al módulo de productos/inventario
interface Producto {
  id: number | string;
  nombre?: string;
  name?: string;
...
// Estructura que devuelve /api/billing/secuenciales/disponibles/
interface SecuencialDisponible {
  empresa_id: number;
  empresa_ruc: string;
...
const isServicioProduct = (p: Producto): boolean => {
  ...
  const tipo = (p.tipo || p.type || "").toString().toUpperCase();
...
const InvoiceWizard: React.FC = () => {
  ...
  useEffect(() => {
    const cargarSecuenciales = async () => {
      ...
      try {
        const url = `/api/billing/secuenciales/disponibles/?empresa=${empresaId}`;
        const resp = await fetch(url, {
          method: "GET",
          headers: {
            Accept: "application/json",
        ...
        if (!resp.ok) {
          throw new Error(
            `Error HTTP ${resp.status} al cargar secuenciales disponibles`,
          );
        }
        const dataRaw: any = await resp.json();
        const list = ensureArray<SecuencialDisponible>(dataRaw);
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      try {
        ...
        params.append("search", term);
        params.append("page_size", "10");
        // Si hay bodega seleccionada, se intenta filtrar por bodega (si el backend lo soporta)
        if (warehouseId) {
  ...
  // =========================
  // Añadir producto como línea (con lectura correcta de stock + IVA estimado)
  // =========================
  const addProductAsLine = async (prod: Producto) => {
    ...
    // Consultar stock siempre que haya bodega seleccionada
    if (warehouseId) {
      try {
        const params = new URLSearchParams();
        // Enviamos ambos nombres de parámetros para ser compatibles con distintos filtros del backend
        params.append("warehouse", String(warehouseId));
        params.append("warehouse_id", String(warehouseId));
  ...
  const totals = useMemo(() => {
    ...
    // IVA de empresa (fallback cuando la línea no tiene uno propio)
  ...
  const isFechaEmisionValida = (): boolean => {
    ...
    // No permitir fecha futura en el wizard (evita FECHA EMISIÓN EXTEMPORANEA por futuro)
  ...
  const canGoNextFromStep1 = (): boolean => {
    ...
    // Si se va a descontar inventario, obligamos a definir bodega desde el Paso 1
  ...
  const handleConfirmar = async () => {
    ...
    // Determinar si hay líneas que requieren inventario
    ...
    // Totales a nivel de factura para el InvoiceSerializer
    const total_sin_impuestos = totals.total; // suma de (cantidad*precio - descuento)
    const total_descuento = totals.descuentoTotal;
    const propina = 0;
    const importe_total = totals.total + totals.impuestos; // el backend recalculará definitivamente

    // Campos snapshot de comprador según InvoiceSerializer / modelo Invoice
    ...
    const payload = {
      ...
      // DRF InvoiceSerializer espera "cliente", no "cliente_id"
      ...
      email_comprador,
      telefono_comprador,
      // El backend recalculará totales/impuestos, pero necesita coherencia básica con las líneas
      lines: lines.map((ln) => {
        const base =
        ...
        return {
          // InvoiceLineSerializer espera "producto", no "product"
  ...
  return (
    <div className="px-4 py-4 md:py-6">
      ...
      <div className="mx-auto mt-4 max-w-5xl">
        {/* Indicador simple de pasos (mobile-first) */}
