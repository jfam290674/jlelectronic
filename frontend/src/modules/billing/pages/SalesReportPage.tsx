 ...
 *
 * VERSIÓN CORREGIDA - Bugs críticos resueltos:
 * ✅ 1. Búsqueda de clientes funciona inmediatamente (reporte inicial automático)
 * ✅ 2. Cliente seleccionado persiste hasta limpiar filtros
 * ✅ 3. Tabs separados correctamente - NO muestra datos incorrectos
 * ✅ 4. Auto-genera reporte al cambiar de tab
 * ✅ 5. Tab "Anulaciones" muestra SOLO facturas ANULADAS
 */
...
export default function SalesReportPage(): React.ReactElement {
  ...
  // ========================================================================
  // EFECTOS - Carga inicial automática (para tener clientes disponibles)
  // ========================================================================

  React.useEffect(() => {
    if (!initialLoadDone && empresaId > 0) {
      setInitialLoadDone(true);
      // Generar reporte inicial de efectivas para tener clientes disponibles
  ...
  // ========================================================================
  // EFECTOS - Búsqueda de clientes en tiempo real (mejorada)
  // ========================================================================
  ...
  React.useEffect(() => {
    const buscar = () => {
      ...
      // Si no hay query, ocultar sugerencias
      if (query.length < 2) {
      ...
      // Buscar en el pool completo de clientes
      ...
      resultados.sort((a, b) => {
        ...
        // Prioridad 1: RUC empieza con query
        if (aRuc.startsWith(q) && !bRuc.startsWith(q)) return -1;
        if (!aRuc.startsWith(q) && bRuc.startsWith(q)) return 1;

        // Prioridad 2: Nombre empieza con query
        if (aNombre.startsWith(q) && !bNombre.startsWith(q)) return -1;
        if (!aNombre.startsWith(q) && bNombre.startsWith(q)) return 1;

        // Default: orden alfabético por nombre
   ...
   *   - false => respeta el filtro de "Estado SRI" del formulario
   */
  async function fetchReport(
    ...
    // Estado:
    // - Para anuladas (soloAnuladas=true): ANULADO (indica factura anulada por usuario)
    // - Para el resto: se utiliza el filtro de Estado SRI del formulario
    if (soloAnuladas) {
      (filters as any).estado = "ANULADO";
    } else if (estado) {
      (filters as any).estado = estado;
    }

    // Si hay cliente seleccionado, filtrar por identificación
    if (clienteSeleccionado) {
      (filters as any).identificacion_comprador =
  ...
  async function handleGenerateReport() {
    ...
    try {
      if (activeTab === "efectivas") {
        console.log(
          "[SalesReport] Generando reporte EFECTIVAS (incluir_anuladas=false)",
        );
        const data = await fetchReport(false, false);
        setReportEfectivas(data);
        console.log("[SalesReport] Reporte efectivas generado:", data.summary);
      } else if (activeTab === "anuladas") {
        console.log(
          "[SalesReport] Generando reporte ANULADAS (solo anuladas, incluir_anuladas=true)",
        );
        const data = await fetchReport(true, true);
        setReportAnuladas(data);
        console.log("[SalesReport] Reporte anuladas generado:", data.summary);
      } else if (activeTab === "comparativa") {
        console.log(
          "[SalesReport] Generando ambos reportes para comparativa (efectivas vs anuladas)",
        );
        const [efectivas, anuladas] = await Promise.all([
          // Efectivas: excluir anuladas
          fetchReport(false, false),
          // Anuladas: solo estado=ANULADO
  ...
  // ========================================================================
  // FUNCIONES - Cambio de tabs (NO limpia reportes)
  // ========================================================================
  ...
  function handleTabChange(tab: ReportTab) {
    ...
    // Auto-generar reporte si no existe para este tab
    if (tab === "efectivas" && !reportEfectivas) {
      console.log(
        "[SalesReport] Auto-generando reporte de efectivas (includeVoided=false)",
      );
      ...
      console.log(
        "[SalesReport] Auto-generando reporte de anuladas (solo anuladas)",
      );
      void fetchReport(true, true)
        .then(setReportAnuladas)
        .catch(console.error);
    } else if (tab === "comparativa") {
      // En comparativa, generar ambos si faltan
  ...
  const currentReport = React.useMemo(() => {
    ...
    // En comparativa, no hay "current report" único
    return null;
  ...
  return (
    <div className="mx-auto flex max-w-7xl flex-col gap-4 p-4 sm:p-6">
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            ...
            <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-emerald-50 to-emerald-100/50 p-4 shadow-sm">
              ...
              <p className="mt-1 text-xs text-slate-600">Con IVA</p>
            </div>
          </div>
      ...
      {/* Vista comparativa (si activeTab === 'comparativa') */}
      {hasComparativa && (
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
