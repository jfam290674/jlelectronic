 ...
 *   - Anular (AUTORIZADA)
 *   - Cancelar (no autorizada / interna)
 *
...
const InvoiceSriActions: React.FC<InvoiceSriActionsProps> = ({
  ...
  const { canEmitir, canAutorizar, canReenviar, canAnular, canCancelar } =
    useMemo(() => {
      ...
      const emitirPermitidos = new Set([
        "BORRADOR",
        "ERROR",
        "NO_AUTORIZADO",
        "PENDIENTE_ENVIO", // el backend lo usa aunque no esté en el Enum base
        "PENDIENTE",
      ]);

      const autorizarPermitidos = new Set([
        "RECIBIDO",
        "EN_PROCESO",
        "NO_AUTORIZADO",
        "ENVIADO",
      ...
      const reenviarPermitidos = new Set([
        "BORRADOR",
        "ERROR",
        "NO_AUTORIZADO",
        "PENDIENTE_ENVIO",
  ...
  const handleUpdated = (data: SriActionResponse | any) => {
    // El padre (InvoiceDetail) se encarga de fusionar y refrescar
    onUpdated?.(data);
  };
  ...
  const handleAnular = async () => {
    ...
    const confirmar = window.confirm(
      "Se anulará legalmente esta factura AUTORIZADA. Esta acción no se puede deshacer. ¿Desea continuar?",
    );
    if (!confirmar) return;
  ...
  const handleCancelar = async () => {
    ...
    const confirmar = window.confirm(
      "Se cancelará internamente esta factura (no autorizada / no enviada). ¿Desea continuar?",
    );
    if (!confirmar) return;
    ...
    try {
      ...
      handleUpdated(data);
      notifySuccess("Factura cancelada correctamente.");
    } catch (err) {
  ...
  const baseButtonClass =
    "inline-flex items-center justify-center px-3 py-1.5 text-xs font-medium rounded-md border transition-colors disabled:opacity-50 disabled:cursor-not-allowed";

  return (
    <div
      className={`flex flex-col gap-2 text-xs text-slate-800 ${className}`}
    >
      {/* Estado actual */}
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        {canAutorizar && (
          <button
            ...
            disabled={disabled}
            className={`${baseButtonClass} border-sky-600 text-sky-800 hover:bg-sky-50`}
          >
        ...
        {canAnular && (
          <button
            ...
            disabled={disabled}
            className={`${baseButtonClass} border-red-500 text-red-700 hover:bg-red-50`}
          >
        ...
        {canCancelar && (
          <button
            ...
            disabled={disabled}
            className={`${baseButtonClass} border-amber-500 text-amber-700 hover:bg-amber-50`}
          >
      ...
      {lastMessage && (
        <div className="mt-1 rounded-md border border-slate-200 bg-slate-50 px-2 py-1 text-[11px] leading-snug text-slate-700">
          {lastMessage}
        </div>
      )}
    </div>
